...
def logIn():
    ...
    for row in cursorLogIn.fetchall():
        encode = jwt.encode({'id':row[0]},secretKey,algorithm='HS256').decode('utf-8')
        # print(encode)
    ...
    # print(dataUser)
    dataUser[0].update({'kode':str(encode)})
...
def readTweet():
    ...
    for row in cursorReadTweet.fetchall():
        ...
        datakode.append(encode)
        dataTweet.append(dict(row))

    for index,token in enumerate(datakode):
        dataTweet[int(index)].update({"kode":str(token)})
...
def readTweetProfile():
    kode = request.json['id']
    decode = jwt.decode(kode,secretKey,'utf-8')
    id = (decode['id'])
...
@app.route('/getHomeNavbar', methods = ['POST'])
def getHomeNavbar():
    kode = request.json['id']
    decode = jwt.decode(kode,secretKey,'utf-8')
    id = (decode['id'])
...
def addFollow():
    if request.method == 'POST':
        ...
        curInsertFollow.execute("Insert into follow(person_id,following) values (%s,%s)",(id,idfollowing))
        con.commit()
...
def editProfile():
    ...
    cursorEditProfile.execute("Update person set username=%s, fullname=%s, email=%s, bio=%s where id=%s",(username,fullname,email,bio,id))
    con.commit()
...
def editPassword():
    ...
    checkingPassword.execute("Select * from person where id = %s and password = %s ",(id, currentPass))
    jml = checkingPassword.rowcount
    
    if (jml > 0):
        if verifyPass == newPass:
            currentEditPass = con.cursor()
            currentEditPass.execute("Update person set password=%s where id = %s and password = %s",(newPass, id, currentPass))
            con.commit()
